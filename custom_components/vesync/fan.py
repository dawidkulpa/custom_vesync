...
class VeSyncFanHA(VeSyncDevice, FanEntity):
    ...
    def __init__(self, fan, coordinator) -> None:
        ...
        if self.smartfan.fan_levels:
            self._speed_range = (1, max(self.smartfan.fan_levels))
        if self.smartfan.modes:
    ...
    def extra_state_attributes(self):
        ...
        attr = {}
        state_dict = self.smartfan.state.to_dict()
    ...
    async def async_set_percentage(self, percentage):
        ...
        await self.smartfan.set_manual_mode()
        await self.smartfan.set_fan_speed(
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        if preset_mode == VS_MODE_AUTO:
            await self.smartfan.set_auto_mode()
        elif preset_mode == VS_MODE_SLEEP:
            await self.smartfan.set_sleep_mode()
        elif preset_mode == VS_MODE_MANUAL:
            await self.smartfan.set_manual_mode()
        elif preset_mode == VS_MODE_TURBO:
            await self.smartfan.set_turbo_mode()
